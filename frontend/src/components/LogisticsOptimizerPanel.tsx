...
export default function LogisticsOptimizerPanel() {
  ...
  return <div className="space-y-6">
    <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6">
      <h3 className="text-base font-semibold text-[#1a2e1a]">Shipment Plan</h3>
      <p className="text-xs text-gray-400 mt-1 mb-5">Compare direct, shared pickup, and collection-hub delivery models.</p>
      <div className="grid sm:grid-cols-3 gap-3">
        <label className="text-xs text-gray-500">Crop<input value={crop} onChange={(e) => setCrop(e.target.value)} className="mt-1 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" /></label>
        <label className="text-xs text-gray-500">Default farmer price (₹/kg)<input type="number" value={basePrice} onChange={(e) => setBasePrice(Number(e.target.value))} className="mt-1 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" /></label>
        <label className="text-xs text-gray-500">Maximum landed cost (₹/kg)<input type="number" value={budget} onChange={(e) => setBudget(Number(e.target.value))} className="mt-1 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" /></label>
      </div>
    </div>

    <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6">
      <div className="flex justify-between items-center mb-4"><div><h3 className="text-base font-semibold text-[#1a2e1a]">Farmer Pickups</h3><p className="text-xs text-gray-400 mt-1">Quantity, price, reliability, and coordinates for each pickup.</p></div><button onClick={addFarmer} className="px-3 py-2 bg-[#e8f0eb] text-[#2d5a3d] rounded-lg text-xs font-semibold">+ Add farmer</button></div>
      <div className="space-y-3">{farmers.map((farmer, index) => <div key={`${farmer.name}-${index}`} className="grid sm:grid-cols-2 lg:grid-cols-7 gap-2 bg-gray-50 border border-gray-100 rounded-xl p-3">
        ...
        <div className="lg:col-span-7 text-[10px] text-gray-400">{farmer.quantity_kg.toLocaleString("en-IN")} kg · ₹{farmer.price_per_kg}/kg · Reliability {farmer.reliability_score}/100 · {farmer.lat}, {farmer.lon}</div>
      </div>)}</div>
    </div>

    <div className="grid lg:grid-cols-2 gap-6">
      <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6"><h3 className="text-sm font-semibold text-[#1a2e1a] mb-3">Buyer Destination</h3><div className="grid grid-cols-3 gap-2"><input value={buyer.name} onChange={(e) => setBuyer({ ...buyer, name: e.target.value })} className="border border-gray-200 rounded-lg px-2 py-2 text-xs" /><input type="number" step="0.01" value={buyer.lat} onChange={(e) => setBuyer({ ...buyer, lat: Number(e.target.value) })} className="border border-gray-200 rounded-lg px-2 py-2 text-xs" /><input type="number" step="0.01" value={buyer.lon} onChange={(e) => setBuyer({ ...buyer, lon: Number(e.target.value) })} className="border border-gray-200 rounded-lg px-2 py-2 text-xs" /></div></div>
      <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6"><label className="flex items-center gap-2 text-sm font-semibold text-[#1a2e1a] mb-3"><input type="checkbox" checked={useHub} onChange={(e) => setUseHub(e.target.checked)} /> Include collection hub</label><div className="grid grid-cols-3 gap-2 opacity-100"><input disabled={!useHub} value={hub.name} onChange={(e) => setHub({ ...hub, name: e.target.value })} className="border border-gray-200 rounded-lg px-2 py-2 text-xs disabled:opacity-40" /><input disabled={!useHub} type="number" step="0.01" value={hub.lat} onChange={(e) => setHub({ ...hub, lat: Number(e.target.value) })} className="border border-gray-200 rounded-lg px-2 py-2 text-xs disabled:opacity-40" /><input disabled={!useHub} type="number" step="0.01" value={hub.lon} onChange={(e) => setHub({ ...hub, lon: Number(e.target.value) })} className="border border-gray-200 rounded-lg px-2 py-2 text-xs disabled:opacity-40" /></div></div>
    </div>

    <button onClick={optimize} disabled={loading || farmers.length === 0} className="px-7 py-3 rounded-xl bg-[#2d5a3d] text-white text-sm font-semibold disabled:opacity-50">{loading ? "Comparing routes..." : "Optimize Logistics & Buyer Cost"}</button>
    {error && <div className="p-3 bg-red-50 border border-red-200 rounded-xl text-xs text-red-700">{error}</div>}

    {result && <>
      <div className="bg-[#244c34] text-white rounded-2xl p-6 shadow-sm"><div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4"><div><div className="text-xs text-green-200">Recommended model</div><h3 className="text-xl font-bold mt-1">{modelLabel(result.logistics_result.recommended_model.name)}</h3><p className="text-xs text-green-100 mt-2 max-w-2xl leading-5">{result.logistics_result.recommended_model.reason}</p></div><div className="grid grid-cols-2 gap-5 text-right"><div><div className="text-2xl font-bold">{money(result.logistics_result.recommended_model.total_cost)}</div><div className="text-[10px] text-green-200">transport cost</div></div><div><div className="text-2xl font-bold">₹{result.logistics_result.recommended_model.cost_per_kg.toFixed(2)}</div><div className="text-[10px] text-green-200">per kg</div></div></div></div></div>
      <div className="grid lg:grid-cols-3 gap-4">{Object.values(result.logistics_result.models).map((model) => <div key={model.model} className={`bg-white border rounded-2xl p-5 shadow-sm ${model.model === result.logistics_result.recommended_model.name ? "border-green-400" : "border-gray-100"}`}><div className="flex justify-between gap-2"><h4 className="text-sm font-semibold text-[#1a2e1a]">{modelLabel(model.model)}</h4>{model.model === result.logistics_result.recommended_model.name && <span className="text-[10px] bg-green-100 text-green-700 rounded px-2 py-1">BEST</span>}</div><div className="grid grid-cols-2 gap-2 mt-4 text-xs"><div className="bg-gray-50 rounded-lg p-2"><span className="text-gray-400">Cost</span><div className="font-semibold">{money(model.total_cost)}</div></div><div className="bg-gray-50 rounded-lg p-2"><span className="text-gray-400">Cost/kg</span><div className="font-semibold">₹{model.cost_per_kg.toFixed(2)}</div></div><div className="bg-gray-50 rounded-lg p-2"><span className="text-gray-400">ETA</span><div className="font-semibold">{model.eta_hours.toFixed(1)} hr</div></div><div className="bg-gray-50 rounded-lg p-2"><span className="text-gray-400">Utilization</span><div className="font-semibold">{model.weight_utilisation_pct.toFixed(1)}%</div></div><div className="bg-gray-50 rounded-lg p-2"><span className="text-gray-400">Trips</span><div className="font-semibold">{model.trips_required}</div></div><div className="bg-gray-50 rounded-lg p-2"><span className="text-gray-400">Vehicle</span><div className="font-semibold truncate">{model.vehicle || model.main_vehicle}</div></div></div></div>)}</div>
      <div className="grid lg:grid-cols-2 gap-6"><div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm"><h3 className="text-sm font-semibold text-[#1a2e1a] mb-4">Final Buyer Cost</h3><div className="space-y-2">{Object.entries(result.buyer_quote.breakdown).map(([label, value]) => <div key={label} className="flex justify-between text-xs border-b border-gray-50 pb-2"><span className="text-gray-500 capitalize">{label.replaceAll("_", " ")}</span><span className="font-semibold">{money(value)}</span></div>)}</div><div className="flex justify-between mt-4 pt-4 border-t"><span className="font-semibold">Final buyer cost</span><div className="text-right"><div className="text-xl font-bold text-[#2d5a3d]">{money(result.buyer_quote.final_buyer_cost)}</div><div className="text-xs text-gray-400">₹{result.buyer_quote.final_cost_per_kg.toFixed(2)}/kg</div></div></div></div><div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm"><h3 className="text-sm font-semibold text-[#1a2e1a] mb-4">Landed Cost & Risk</h3><div className="grid grid-cols-2 gap-3"><div className="bg-gray-50 rounded-xl p-3 text-xs"><div className="text-gray-400">Blended landed cost</div><div className="text-lg font-bold text-[#1a2e1a]">₹{result.logistics_result.landed_cost.blended_landed_cost_per_kg.toFixed(2)}/kg</div></div><div className="bg-gray-50 rounded-xl p-3 text-xs"><div className="text-gray-400">Spoilage estimate</div><div className="text-lg font-bold text-[#1a2e1a]">{result.buyer_quote.spoilage_detail.spoilage_rate_pct.toFixed(1)}%</div></div><div className="bg-gray-50 rounded-xl p-3 text-xs"><div className="text-gray-400">Total landed cost</div><div className="text-lg font-bold text-[#1a2e1a]">{money(result.logistics_result.landed_cost.total_landed_cost)}</div></div><div className={`rounded-xl p-3 text-xs ${result.logistics_result.landed_cost.within_buyer_budget ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}><div>Buyer budget</div><div className="text-lg font-bold">{result.logistics_result.landed_cost.within_buyer_budget ? "Within budget" : "Over budget"}</div></div></div></div></div>
    </>}
    <p className="text-[10px] text-gray-400 text-center">Planning estimate. Live road routing falls back to geographic-distance estimation when routing services are unavailable.</p>
  </div>;
}
