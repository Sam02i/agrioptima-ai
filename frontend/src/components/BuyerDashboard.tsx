...
export default function BuyerDashboard({ onHome, onFarmer }: { onHome: () => void; onFarmer: () => void }) {
  ...
  return <div className="buyer-app">
    ...
    <aside className={`buyer-sidebar ${sidebar ? "open" : ""}`}><div className="buyer-sidebar-head"><div className="buyer-brand"><span><Leaf /></span><b>AgriOptimaᴬᴵ</b></div><button aria-label="Close buyer menu" onClick={() => setSidebar(false)}><X /></button></div><div className="workspace-card"><span>AM</span><div><b>Mehta Foods</b><small>Buyer workspace</small></div><ChevronDown /></div><nav><p>Workspace</p>{NAV.map(({ key, label, Icon }) => <button key={key} className={active === key ? "active" : ""} onClick={() => navigate(key)}><Icon /><span>{label}</span>{key === "ranking" && <em>3</em>}</button>)}</nav><div className="buyer-credit-mini"><div><CircleDollarSign /> Procurement credit</div><strong>₹3,80,000</strong><small>Available of ₹5,00,000</small><div className="buyer-credit-track"><i /></div><button onClick={() => navigate("credit")}>View credit profile →</button><button className="buyer-home-link" onClick={onHome}>← Public landing page</button></div></aside>
    <main className="buyer-main"><header className="buyer-topbar"><button className="mobile-nav" aria-label="Open buyer menu" onClick={() => setSidebar(true)}><Menu /></button><div className="global-search"><Search /><input placeholder="Search sellers, orders, shipments…"/><kbd>⌘ K</kbd></div><div className="portal-switch"><button onClick={onHome}>Home</button><button onClick={onFarmer}>Farmer portal</button></div><div className="buyer-person"><button aria-label="Notifications"><Bell /><i /></button><span>AM</span><div><b>Arjun Mehta</b><small>Procurement lead</small></div><ChevronDown /></div></header><div className="buyer-content"><section className="buyer-heading"><div><span>Tuesday, 1 September</span><h1>{COPY[active][0]}</h1><p>{COPY[active][1]}</p></div>{active !== "overview" && <button onClick={() => navigate("overview")}>← Command center</button>}</section>
    ...
    </div><footer className="buyer-footer">AgriOptimaᴬᴵ · Farmer marketplace · Crop intelligence · Freshness · Credit · Supplier ranking · Logistics</footer></main>
  </div>;
}
