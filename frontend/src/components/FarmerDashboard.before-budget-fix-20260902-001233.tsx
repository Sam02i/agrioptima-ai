...
const FARM_NAV: Array<[FarmerTab, string, string]> = [
  ...
  ["listings", "List my produce", "◫"],
  ["orders", "Orders & shipments", "▣"],
];
const FARM_COPY: Record<FarmerTab, [string, string]> = {
  overview: [
    "Farmer command center",
    "Plan crops, prove quality, reach buyers, and manage listings from one workspace.",
  ],
  profile: [
    "Connected farmer data",
    "Review farms, soil context, goals, and registered produce.",
  ],
  crop: [
    "What should I grow?",
    "Get feasible and explainable crop recommendations from your farm conditions.",
  ],
  soil: [
    "Soil health intelligence",
    "Automatically identify missing nutrients and the best fertilizer plan from your Soil Health Card.",
  ],
  freshness: [
    "Check produce freshness",
    "Use the trained tomato model to grade produce before listing or dispatch.",
  ],
  listings: [
    "List produce for buyers",
    "Create a priced produce listing manually and add your own produce image.",
  ],
  orders: [
    "Orders & shipments",
    "See shipped, in-transit, and pending buyer orders from your farm.",
  ],
};
          ...
          {active === "overview" && (
            <>
              <section className="farmer-welcome">
                <div>
                  <span>
                    ✦ {farmer?.name || "Personalized"} farmer workspace
                  </span>
                  <h2>
                    Grow with evidence.
                    <br />
                  ...
                  <p>
                    {farmer
                      ? `${farmer.village}, ${farmer.district} · ${profile?.needs?.primary_goal || "Plan crops, reach buyers, and deliver confidently."}`
                      : "Your soil, farm goals, produce quality, buyer access, and delivery planning are connected."}
                  </p>
                  <button onClick={() => go("crop")}>
                    Build my crop plan →
                  </button>
                </div>
                <div className="farmer-quick">
                  {[
                    [
                      `${farm?.area_acres ?? farmer?.total_area_acres ?? 0}`,
                      "Acres registered",
                    ],
                    [
                      `${profile?.listings.filter((item) => item.listing_status === "AVAILABLE").length ?? farmer?.listing_count ?? 0}`,
                      "Active crop listings",
                    ],
                    [`${farm?.ph ?? "—"}`, "Soil pH"],
                    [
                      `₹${Math.round((profile?.needs?.investment_budget ?? 0) / 1000)}k`,
                      "Investment budget",
                    ],
                  ].map(([v, l]) => (
                    <span key={l}>
              ...
              <section className="model-demo-grid">
                {FARM_NAV.slice(2).map(([key, label, icon]) => (
                  <button key={key} onClick={() => go(key)}>
                    <span>{icon}</span>
                    <div>
                      <b>{label}</b>
                      <small>{FARM_COPY[key][1]}</small>
                    </div>
                    <i>Open demo →</i>
                  </button>
                ))}
                  ...
                  initialValues={
                    farmer && farm
                      ? {
                          ...
                          previous_crop: farm.previous_crop || "Unknown",
                          investment_budget_rupees:
              ...
              <div ref={ref}>
                {results ? (
                  ...
                  <div className="demo-panel empty-workspace">
                    ...
                    <p>
                      Submit the connected farm profile to compare feasible
                      crops and expected economics.
                    </p>
                  </div>
                )}
              </div>
