    ...
    <div className="fixed inset-0 z-[70] overflow-y-auto bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6">
      <div className="relative w-full max-w-3xl overflow-hidden rounded-[2rem] bg-white shadow-2xl border border-white/80">
        ...
        <div className="bg-[#f0fdf4] px-6 py-8 text-center sm:px-10 sm:py-10">
          ...
          <h2 className="mt-4 font-serif text-3xl font-bold text-[#052e16] sm:text-4xl">{copy.question}</h2>
        ...
        <div className="grid gap-4 p-5 sm:grid-cols-2 sm:p-8">
          ...
          >
            ...
            <h3 className="mt-5 font-serif text-2xl font-bold text-[#052e16]">{copy.farmer}</h3>
            <p className="mt-2 text-sm leading-6 text-gray-600">{copy.farmerHelp}</p>
            {!createMode&&farmers.length>0&&<label className="mt-4 block text-xs font-bold uppercase tracking-wider text-[#166534]">{copy.choose}<select value={selected} onChange={(event)=>setSelected(event.target.value)} className="mt-2 w-full rounded-xl border border-emerald-200 bg-white px-3 py-2.5 text-sm font-medium normal-case tracking-normal text-gray-800 outline-none focus:ring-2 focus:ring-emerald-600">{farmers.map((farmer)=><option key={farmer.farmer_id} value={farmer.farmer_id}>{farmer.name} · {farmer.district}</option>)}</select></label>}
            {recordsError&&<small className="mt-3 block text-xs text-amber-700">{recordsError}</small>}
            {!createMode?<div className="mt-5 flex flex-wrap gap-2"><button type="button" onClick={() => onFarmer(selected || undefined)} className="inline-flex items-center gap-2 rounded-full bg-[#166534] px-4 py-2.5 text-sm font-bold text-white">{copy.open} <ArrowRight className="h-4 w-4" /></button><button type="button" onClick={()=>setCreateMode(true)} className="rounded-full border border-emerald-200 px-4 py-2.5 text-sm font-bold text-[#166534]">{copy.create}</button></div>:<form onSubmit={saveFarmer} className="mt-4 grid grid-cols-2 gap-2"><input required placeholder={copy.name} value={form.name} onChange={(e)=>setForm({...form,name:e.target.value})} className="col-span-2 rounded-xl border border-emerald-200 px-3 py-2 text-sm"/><input required placeholder={copy.village} value={form.village} onChange={(e)=>setForm({...form,village:e.target.value})} className="rounded-xl border border-emerald-200 px-3 py-2 text-sm"/><input required placeholder={copy.district} value={form.district} onChange={(e)=>setForm({...form,district:e.target.value})} className="rounded-xl border border-emerald-200 px-3 py-2 text-sm"/><label className="text-xs text-gray-500">{copy.acres}<input required type="number" min="0.1" step="0.1" value={form.area_acres} onChange={(e)=>setForm({...form,area_acres:Number(e.target.value)})} className="mt-1 w-full rounded-xl border border-emerald-200 px-3 py-2 text-sm"/></label><label className="text-xs text-gray-500">{copy.budget}<input required type="number" min="1000" step="1000" value={form.investment_budget_rupees} onChange={(e)=>setForm({...form,investment_budget_rupees:Number(e.target.value)})} className="mt-1 w-full rounded-xl border border-emerald-200 px-3 py-2 text-sm"/></label><button type="submit" disabled={saving} className="col-span-2 rounded-full bg-[#166534] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60">{saving?copy.saving:copy.createOpen}</button><button type="button" onClick={()=>setCreateMode(false)} className="col-span-2 text-xs font-bold text-gray-500">{copy.existing}</button></form>}
          </div>
          ...
          >
            ...
            <h3 className="mt-5 font-serif text-2xl font-bold">{copy.buyer}</h3>
            <p className="mt-2 text-sm leading-6 text-white/75">{copy.buyerHelp}</p>
            <span className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-[#c4f042]">{copy.openBuyer} <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span>
          </button>
        </div>
      </div>
    </div>
